...
combData <- function(dataset, reference, class = c("rgset", "betas"),
    ...
    reference <- reference[, Biobase::pData(reference)$CellType %in% cellTypes]
    if (class == "betas") {
    ...
    combMeta <- data.frame(sampleNames = c(colnames(samp.n), colnames(ref.n)),
                           ...
                           cellType = c(sampCT, as.character(Biobase::pData(reference)$CellType)),
