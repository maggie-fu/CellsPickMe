...
#'
#' @param dataset An RGchannelSet or a beta matrix of the users' DNA methylation
#'  data set
#' @param reference An RGchannelSet of the reference DNA methylation data set,
#' preferentially the output from the `getRef()` function, to ensure the
#' `pData()` formatting is appropriate
#' @param class A character specifying the argument `dataset`'s class, with the
#' option or rgset (for RGChannelSet) or betas (for beta matrix)
#' @param normType A character specifying the method for normalizing user sample
#'  and reference data sets together, with the options of "Noob", "Funnorm",
#'  "Quantile", "Quantile.b", and "None" -- refer to the Details section for
#'  more information on the options.
#' @param cellTypes A vector of characters specifying the cell types to be
#' estimated. If the `getRef()` function was used, use the output as shown in
#' the Example.
#' @param verbose A Boolean, specifying whether the function should be verbose
#'
#' @return A list of data frames, including the sample and reference data post
