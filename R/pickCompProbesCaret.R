...
pickCompProbesCaret <- function(betas, meta, ct, ps = c("any", "both"), min.delta.beta = 0, p.val = 1e-8,
    ...
    set.seed(seed)
    seeds5 <- vector(mode = "list", length = 26) # 5 repeats of 5-fold CV [cannot do 10 - not enough samples] (5*5+1) - change if the sampling method is altered in trainControl below
    for(i in 1:25) seeds5[[i]] <- sample.int(1000, 5)
    seeds5[[26]] <- sample.int(1000, 1)
    seeds9 <- vector(mode = "list", length = 26) # 5 repeats of 5-fold CV [cannot do 10 - not enough samples] (5*5+1) - change if the sampling method is altered in trainControl below
    for(i in 1:25) seeds9[[i]] <- sample.int(1000, 9)
    seeds9[[26]] <- sample.int(1000, 1)
    ...
    for(i in 1:25) seedsGA2[[i]] <- sample.int(1000, 3)
    seedsGA2[[26]] <- sample.int(1000, 1)

    control5 <- caret::trainControl(method = "repeatedcv",
                                    number = 5,
                                    repeats = 5,
                                    # sampling = "up",
                                    verboseIter = F,
                                    seeds = seeds5)
    control10 <- caret::trainControl(method = "repeatedcv",
                                     number = 5,
                                     repeats = 5,
                                     # sampling = "up",
                                     verboseIter = F,
    ...
    control9 <- caret::trainControl(method = "cv",
                                    ...
                                    verboseIter = F,
                                    seeds = seeds9)
    control400 <- caret::trainControl(method = "repeatedcv",
                                      number = 5,
                                      repeats = 5,
                                      # sampling = "up",
                                      verboseIter = F,
                                      seeds = seeds400)
    controlGA <- caret::trainControl(method = "repeatedcv",
                                     number = 5,
                                     repeats = 5,
                                     # sampling = "up",
                                     verboseIter = F,
    ...
    probeList <- lapply(ct, function(ctType) {
        ...
        if ("EL" %in% caretMods){
            ...
            ELtune <- expand.grid(alpha = seq(0.05, 0.95, length = 10),
                                  lambda = seq(0.1, 1, length = 10))
