...
#'
#' @param dataNormed A list of dataframe containing the normalized data, output
#' of `combData()`
#' @param probeList A character, specifying how the probe should be selected.
#' Options include "Ttest", "Caret_CV", "Caret_LOOCV", and "IDOL"
#' @param probeSelect If `probeSelect = Ttest`, specify how should the top
#' probes be picked from T test output, options include "both", "any", "pval"
#' @param nProbes An integer specifying the number of probes to pick for
#' each cell type
#' @param caretMods If `probeSelect %in% c("Caret_CV", "Caret_LOOCV")`, input
#' a vector of characters that specify the models to use for feature selection
#' @param seed An integer specifying the seed for reproducibility
#' @param p.val If `probeSelect = Ttest`, specify a numeric value for maximum
#' pvalue cutoff
#' @param min.delta.beta If `probeSelect = Ttest`, specify a numeric value for
#' minimum delta beta (effect size) cutoff
#' @param filterK If `probeSelect %in% c("Caret_CV", "Caret_LOOCV")`, input an
#' integer representing the number probes to input to the machine learning
#' algorithms with top T test probes
#' @param plotRef A Boolean specifying whether to plot a heatmap showing the
#' clustering performance using the probes selected
#' @param verbose A Boolean specifying whether the function should be verbose
#' or not
#'
#' @return A list containing the features selected by the specified methods,
#' and the coefficients of the selected probes for downstream prediction
#' @export
#'
#' @examples
#' # Load example blood cell mixture, subsetted from IDOL dataset (GSE110554)
...
#' # Combine sample and reference data sets together,
#' followed by normalization (if selected)
