...
The **CellsPickMe** package takes DNA methylation data generated from Illumina microarray and predict its cellular composition based on cell types available in the reference profiles. Currently, the algorithm is compatible for peripheral blood, cord blood, saliva, and brain (neuron vs non-neuron). The table below illustrate the reference datasets available in the **CellsPickMe** package for deconvolution. We curated the UniBlood references (7, 13, and 19) to address the current challenge in deconvoluting longitudinal and pediatric blood samples.  

```{r modelstable, echo=FALSE}
library(knitr)

models = data.frame(Tissue = c("Adult peripheral blood", "Adult peripheral blood", "Adult peripheral blood", "Cord and adult peripheral blood", "Cord and adult peripheral blood", "Cord and adult peripheral blood", "Cord blood", "Saliva", "Brain"),
                    Reference_dataset = c("Reinius (2012)", "IDOL (2016)", "IDOL extended (2022)", "UniBlood7", "UniBlood13", "UniBlood19", "Cord (2018)", "Middleton (2022)", "Brain (2012)")) 

kable(models, caption = 'Available reference datasets for tissues in CellsPickMe')
```

The figure below focused on our optimization of the deconvolution pipeline in the blood tissue. The four main steps of cellular deconvolution are reference selection, data normalization, feature selection, and regression. The function that 

```{r, echo = FALSE, fig.cap="CellsPickMe pipeline"}
knitr::include_graphics("CellsPickMe_pipeline.pdf")
```
...
```{r install CellsPickMe, eval = FALSE}
library(devtools)
